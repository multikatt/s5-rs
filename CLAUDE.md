...
- **Pinning/replication**: If User A shares a file, it's a SPOF on A's storage unless User B explicitly pins it to their own Sia backend. Pinning should be a first-class operation.
...
**Registry setup**: `open_tee_registry()` creates `TeeRegistry(remote, local)` — reads try remote first (freshest state), writes go to both. Skips self in bootstrap peers.

### 3. Snapshot sharing fix
...
### 4. FUSE mount with on-demand fetching
**Command**: `s5 group mount <group> <label> <mountpoint>`

**Flow**:
1. Fetches DirV1 metadata tree recursively from peers (`fetch_dir_metadata_recursive`)
2. Merges snapshot into a temp FS5 root at `/tmp/s5-group-mount/<group_id>/<label>/`
3. Builds a `FallbackBlobStore` — wraps multiple `BlobStore` instances, tries each in order:
...
4. Mounts read-only via `s5_fuse::mount`

**Key design detail**: `FallbackBlobStore` implements `Store` but internally dispatches to `BlobStore::read_as_bytes(hash, ...)` on each store, so each underlying store uses its own path encoding (base32 for local, base64url for remote).

### 5. Group pin command
**Command**: `s5 group pin <group> <label> [--path <path>] [--store <name>]`

Downloads file content blobs from peers and stores them locally so this node can serve them to others. Supports pinning a single file, a directory, or the entire shared root.

### 6. Default peer policy (`peer_default`)
...
**Required node config for group sharing**:
```toml
[peer_default.blobs]
...
- Run while the server is stopped (e.g., `group create`)
...
5. **Better error handling**: BlobsServer silently returns empty responses for many error conditions (blob not found, unauthorized). Could return proper error codes so clients can distinguish "not found" from "not authorized".

6. **Concurrent pinning**: `group pin` downloads files sequentially. Could parallelize for large shares.
...
- Key crates:
  ...
  - `s5_registry` — iroh-based mutable registry (distributed key-value store)
  ...
  - `s5_groups` — group model, state management, invites (NEW)
